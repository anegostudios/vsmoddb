...
$oneClickInstallWorks = isOneClickInstallPlatform();
...
$view->assign("changelogColspan", 5 + (($asset['category'] & CATEGORY__MASK) === CATEGORY_GAME_MOD ? ($oneClickInstallWorks ? 2 : 1) : 0), null, true);
$view->assign("isFollowing", empty($user) ? 0 : $con->getOne('SELECT modId FROM userFollowedMods WHERE modId = ? AND userId = ?', [$asset['modId'], $user['userId']]), null, true);
